...
using Serilog;
using Teldat.Reporting.Server.Infrastructure;
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            services.AddMvc()
                .AddRazorOptions(options =>
                {
                    ...
                    options.ViewLocationFormats.Add("/Features/Shared/{0}");
                })
                .AddFluentValidation(options => options.RegisterValidatorsFromAssemblyContaining<Startup>());

            services.AddScoped(typeof(IPipelineBehavior<,>), typeof(RequestLogging<,>));
            services.AddScoped(typeof(IPipelineBehavior<,>), typeof(RequestValidation<,>));
            services.AddScoped(typeof(IPipelineBehavior<,>), typeof(RequestTransactionAndEventing<,>));
